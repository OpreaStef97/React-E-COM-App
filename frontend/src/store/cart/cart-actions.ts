...
import { CartType, cartActions } from './cart-slice';

const updateCart = async (cart: CartType, csrfToken: string) => {
    const res = await fetch(`${process.env.REACT_APP_API_URL}/carts`, {
        method: 'PUT',
        body: JSON.stringify({
            products: (cart as CartType).items.map(({ id, quantity }) => {
                return {
                    product: id,
                    quantity,
                };
...
export function sendCartData(cart: CartType, csrfToken: string) {
    ...
    return async (dispatch: Dispatch) => {
        ...
        try {
            ...
            if (
                ...
                if ((lcart as CartType).totalAmount < 0 || (lcart as CartType).totalQuantity < 0) {
